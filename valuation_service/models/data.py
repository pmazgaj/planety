...
class Product:
    ...
    def __init__(self, obj_id, price, currency, quantity, matching_id):
        self.id = int(obj_id)
        self.price = float(price) if price.lstrip('-').replace('.', '', 1).isdigit() else 1.0
        self.currency = currency
        self.quantity = float(quantity) if quantity.lstrip('-').replace('.', '', 1).isdigit() else 1.0
    ...
    def __str__(self):
        return "id: {} price: {} currency: {} " \
               "quantity: {} matching_id: {}, total_price: {} converted price: {}".format(self.id, self.price,
